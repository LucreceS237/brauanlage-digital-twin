"""
File: opcua_mapping.py
Work Package: AP4
Responsible Engineer: Engineer C
Purpose: AP4 FSM module: opcua_mapping.
"""
...
SIGNAL_CONTRACT: tuple[OpcUaSignal, ...] = (
    ...
    OpcUaSignal("durchfluss_nachguss_maische", "flow_k3_to_k1_l_min", True, "Durchfluss K3 nach K1", "L/min"),
    OpcUaSignal("k3_temperatur", "k3_temperature_c", True, "Temperatur Nachgussbehälter K3", "°C"),
    OpcUaSignal("k3_fuellstand", "k3_level_l", True, "Füllstand Nachgussbehälter K3", "L"),
    OpcUaSignal("k3_maximaler_fuellstand", "k3_level_max", True, "Maximaler Füllstand K3"),
    OpcUaSignal("k3_minimaler_fuellstand", "k3_level_min", True, "Minimaler Füllstand K3"),
    OpcUaSignal("k1_temperatur", "k1_temperature_c", True, "Temperatur Maischebehälter K1", "°C"),
    OpcUaSignal("k1_fuellstand", "k1_level_l", True, "Füllstand Maischebehälter K1", "L"),
    OpcUaSignal("k1_fuellstand_voll", "k1_level_full", True, "Vollmeldung K1"),
    OpcUaSignal("k2_temperatur", "k2_temperature_c", True, "Temperatur Läuter-/Kochbehälter K2", "°C"),
    OpcUaSignal("k2_fuellstand", "k2_level_l", True, "Füllstand K2", "L"),
...
def validate_required_signals(raw_values: dict[str, Any]) -> list[Diagnostic]:
    ...
    if not missing:
        return []
    return [make_diagnostic(Severity.ERROR, FaultCode.ERROR_020_AP3_REQUIRED_SIGNAL_MISSING, "missing_api_names", ",".join(missing), "all required api names")]
    ...
    for sheet in workbook.worksheets:
        ...
        for row in rows[1:]:
            ...
            if api and str(api).lower() in lower_required:
                ...
                signals.append(OpcUaSignal(base.api_name, base.canonical_name, True, base.description, str(unit) if unit is not None else base.unit, str(node) if node is not None else None))
